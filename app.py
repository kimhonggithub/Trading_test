...
def get_eth_data(interval="1h", lookback="30 days ago UTC"):
    klines = client.get_historical_klines("ETHUSDT", interval, lookback)
    df = pd.DataFrame(klines, columns=[
        'timestamp', 'open', 'high', 'low', 'close', 'volume',
        'close_time', 'quote_asset_volume', 'number_of_trades',
        'taker_buy_base_asset_volume', 'taker_buy_quote_asset_volume', 'ignore'
...
def prepare_lstm_input(df, prediction_days=20):
    close_prices = df['close'].values.reshape(-1, 1)
    scaled_data = scaler.fit_transform(close_prices)  # use saved scaler
    x_input = []
    for i in range(prediction_days, len(scaled_data)):
        x_input.append(scaled_data[i-prediction_days:i, 0])
    x_input = np.array(x_input)
    x_input = np.reshape(x_input, (x_input.shape[0], x_input.shape[1], 1))
    return x_input, scaled_data

def predict_future_days(last_scaled, days=5, prediction_days=20):
    future_preds = []
    current_input = last_scaled.copy()
    for _ in range(days):
        x_input = np.reshape(current_input, (1, prediction_days, 1))
        pred_scaled = model.predict(x_input)
        pred_price = scaler.inverse_transform(pred_scaled)[0][0]
        future_preds.append(pred_price)
        current_input = np.append(current_input[1:], [[pred_scaled[0][0]]], axis=0)
    return future_preds
...
get_data_button = st.sidebar.button("Fetch Data & Predict")
...
st.title("ETHUSDT LSTM Price Prediction")



if get_data_button:
    eth_df = get_eth_data(interval=interval, lookback=lookback)
    
    # Prepare LSTM input
    x_input, scaled_data = prepare_lstm_input(eth_df)
    
    # Predict past trend (aligned with ETH data)
    predicted_prices = model.predict(x_input)
    predicted_prices = scaler.inverse_transform(predicted_prices)
    
    # Predict next 5 days
    last_20_scaled = scaled_data[-20:]  # last 20 days
    next_5_days = predict_future_days(last_20_scaled, days=5)
    future_dates = pd.date_range(start=eth_df['timestamp'].iloc[-1], periods=6, freq='D')[1:]
    ...
    # Predicted prices (aligned with actuals)
    fig.add_trace(go.Scatter(
        x=eth_df['timestamp'].iloc[-len(predicted_prices):],
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Next 5-Day Prediction'
    ))
    
    fig.update_layout(
        title="ETHUSDT Price Prediction (LSTM)",
        xaxis_title="Date",
    ...
    # Show next 5-day prices in table
    st.subheader("Next 5-Day Predicted Close Prices")
    st.table(pd.DataFrame({
        "Date": future_dates,
        "Predicted Close": next_5_days
    }))
